...
namespace SocialForcesGlobals
{
	...
	SteerSuiteGridEnvironment * gEnvironmentForAStar;
...
void SocialForcesAIModule::init( const SteerLib::OptionDictionary & options, SteerLib::EngineInterface * engineInfo )
{
	...
	_gEngine = engineInfo;
	SteerLib::GridDatabase2D * grid = dynamic_cast<SteerLib::GridDatabase2D *>(_gEngine->getSpatialDatabase());
	{
		gEnvironmentForAStar = new SteerSuiteGridEnvironment(grid);
	}


	// gEngine = engineInfo;
	SocialForcesGlobals::gSpatialDatabase = _gEngine->getSpatialDatabase();
...
Util::PerformanceProfiler *  SocialForcesAIModule::getAIProfiler() { return &gPhaseProfilers->aiProfiler; }