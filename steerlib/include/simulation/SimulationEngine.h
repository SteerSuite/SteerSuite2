...
/*
	...
	*/
	...
	public:
		...
		void finish();
		/// Saves simulation stats for users if the -_saveSimData flag is set
		void saveSimulationStats();
		...
		virtual unsigned int getRandomSeed() { return _randomSeed; }
	...
	protected:
		...
		/// @name Simulation Traces and Crowd Flow Info.
		//@{		
		float _sim_start_time;
		float _sim_end_time;
		float _traces_time_since_last_step;
		float _traces_last_step_time;
		std::ofstream _traces_out;
		unsigned int _agents_completed;
		//@}
