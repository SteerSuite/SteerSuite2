...
void AgentInterface::updateLocalTarget()
{
	...
	Util::Point tmpTarget;
	// std::cout << "Size of mid term path: " << this->_midTermPath.size() << std::endl;

	// Check to see if we have lost sight of the current target
	// This means we have been pushed off path, if:
	// we are using a stering module that is not predicated on planning, and 
	// the environment is concave this will lead to a stuck condition
	// so we must replan
	...
	// Update local target
	int i=0;
	for (i=0 ;i < this->_midTermPath.size() && (i < FURTHEST_LOCAL_TARGET_DISTANCE); i++ )
	{
	...
	for (int j=0; j < this->_midTermPath.size() && j < i && 
				  j < FURTHEST_LOCAL_TARGET_DISTANCE && 
				  _midTermPath.size() > 1; j++ )
	{
			_midTermPath.pop_front();
	}
}
...
bool AgentInterface::reachedLocalTarget()
{
