...
void SimulationEngine::_reset()
{
	...
	// initializing simulation traces info	
	_sim_start_time = 0.0;
	_sim_end_time = 0.0;
	_traces_time_since_last_step = 0.0;
	_traces_last_step_time = 0.0;
	_agents_completed = 0;
}
...
void SimulationEngine::init(SimulationOptions * options, SteerLib::EngineControllerInterface * engineController)
{
	...
	_randomSeed = options->globalOptions._random_seed;
	_saveSimData = options->globalOptions.dataFileName;
	_testcase = options->moduleOptionsDatabase["testCasePlayer"]["testcase"];

	if (_options->globalOptions.saveTraces)
		_traces_out.open(_options->globalOptions.tracesFileName, ios::out | ios::trunc);
	...
	if ( _options->planningDomainOptions.name == "gridDomain")
	{
		...
		//TODO should make this a module!!
...
void SimulationEngine::finish()
{
	...
	else if ((_options->spatialDatabaseOptions.name == "kdTreeDatabase") && _spatialDatabase != NULL)
	{
		// delete _spatialDatabase;
	}

	if ((_options->planningDomainOptions.name == "gridDomain") && _pathPlanner != NULL)
	{
		delete _pathPlanner;
	}
...
void SimulationEngine::postprocessSimulation()
{
	...
	// save simulation stats for user if _saveSimData and _testcase are set - MU
	if (_saveSimData != "" && _testcase != "")
		saveSimulationStats();
...
bool SimulationEngine::update( bool advanceRealTimeOnly )
{
	...
	if (!advanceRealTimeOnly) {
		...
		if (_simulateOneStep() == true) {
			...
			if (_options->globalOptions.saveTraces)
			{
				_sim_end_time = _clock.getCurrentSimulationTime();
				_traces_out << "\n" << (_agents_completed / ((_sim_end_time - _sim_start_time) * 1.0));
				_traces_out.close();
			}
...
bool SimulationEngine::_simulateOneStep()
{
	...
	float currentRTime = _clock.getCurrentRealTime();
	float timeDiff = currentRTime - _traces_last_step_time;
	_traces_time_since_last_step += timeDiff;
	_traces_last_step_time = currentRTime;
	if (_options->globalOptions.saveTraces && (_traces_time_since_last_step >= _options->globalOptions.traceStepSize))
	{
		dumpTrace = true;
		_traces_time_since_last_step = 0.0;
	}
	...
	for ( agentIterator = _agents.begin(); agentIterator != _agents.end(); ++agentIterator )
	{
		if ((*agentIterator)->enabled()){
			...
			if (dumpTrace)
			{
				Util::Point _p = (*agentIterator)->position();
				Util::Vector _v = (*agentIterator)->velocity();
				double _vel = std::sqrt((_v.x*_v.x) + (_v.y*_v.y) + (_v.z*_v.z));
				_traces_out << _p.x << "," << _p.y << "," << _p.z << "," << ((ceilf(_vel * 1000) / 1000.0)) << ";"; // storing position and velocity of the agents
			}
		}
		else {
			if(dumpTrace)
				_traces_out << -1 << "," << -1 << "," << -1 << "," << -1 << ";"; // traces placeholder for the completed agents
			//for some reasons, finished() returns false if it has been disabled because at that time _enabled is false. So negating the condition - MU
			if((*agentIterator)->finished()) {	//for most AIs, this will in turn call enabled() and duplicate original behavior; ShadowAI overrides this behavior
				numDisabledAgents++;
				_agents_completed = numDisabledAgents;
			}
			if(iter <= ((int)_spawned_agent_emitter_num.size())-1) // subtracting 1 from size causes underflow issues
			{//make sure agent is within bounds
	...
	// insert newline for next set of traces
	if(dumpTrace)
		_traces_out << "\n";

	// emit agents and turn off disabled agent from emitting more agents
	int j = 0;
	for(j = 0; j < (int) agentsEmit.size(); j++) {
...
#if __APPLE__ 
		cerr << "Warning: Graphics engine does not support "  << _options->guiOptions.lineWidth 
			<< " line width\n" ;
	 glLineWidth(1.0) ;
#endif
	checkError() ;
...
void SimulationEngine::resizeGL(int width, int height)
{
	...
	glm::mat4 m = glm::perspective(glm::radians(fieldOfViewY), aspectRatio, hither, yon);
	DrawLib::setProjectionMatrix(m);
...
SteerLib::ModuleMetaInformation * SimulationEngine::_loadModule(const std::string & moduleName, const std::string & searchPath)
{
	...
	if (newModule == NULL) {
...
#endif
		char delim = ';';
		std::vector<string> paths = split(searchPath,delim);
		...
		paths = split(_options->engineOptions.moduleSearchPath, delim);
		for (size_t p=0; (!foundModule) && (p < paths.size()); p++)
		{
			moduleFileName = paths[p] + libstr + moduleName + extension;
			std::cout << "checking: " << moduleFileName << " for module" << std::endl;
...
{
	newObstacle->setColour(_options->guiOptions.obstacleColor);
...
void SimulationEngine::saveSimulationStats()
{
	std::cout << "Testcase: " << _testcase << "\nsaveSimData: " << _saveSimData << std::endl;

	Logger * simulationLogger = LogManager::getInstance()->createLogger(_saveSimData, LoggerType::BASIC_APPEND);

	// adding data fields
	simulationLogger->addDataField("random_seed", DataType::Integer);
	simulationLogger->addDataField("frames", DataType::Integer);
	simulationLogger->addDataField("testcase", DataType::String);
	...
	DataItem d1;
	d1.integerData = _randomSeed;
	simulationLogObject.addLogDataItem(d1);

	DataItem d2;
	d2.integerData = _numFramesSimulated;
	simulationLogObject.addLogDataItem(d2);

	DataItem d3;
	d3.string = _testcase;
	simulationLogObject.addLogDataItem(d3);
