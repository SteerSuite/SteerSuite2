...
void GridDatabase2D::draw()
{
	...
	if (_glGeom == NULL) {
		// make the geometry
		_glGeom = DrawLib::MakeGLGrid("GridDataBase", _xOrigin, _zOrigin, _xGridSize, _zGridSize, _xNumCells, _zNumCells);
		if (_glGeom == NULL) {
			cerr << "ERROR: GridDatabase2D::GridDatabase2D() Cannot allocate _glGeom\n";
			exit(-1);
		}
	}

	DrawLib::setColour(1.0, 1.0, 1.0);
	DrawLib::disableLights();
