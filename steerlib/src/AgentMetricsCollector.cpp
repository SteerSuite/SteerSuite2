...
bool AgentMetricsCollector::_updateCollisionStats(SpatialDataBaseInterface * gridDB, AgentInterface * updatedAgent, float currentTimeStamp)
{
	//
...
void AgentMetricsCollector::update(SpatialDataBaseInterface * gridDB, SteerLib::AgentInterface * updatedAgent, float currentTimeStamp, float timePassedSinceLastFrame)
{
	...
	bool isColliding = _updateCollisionStats(gridDB, updatedAgent, currentTimeStamp);
	if (isColliding) _metrics.timeSpentInContact += timePassedSinceLastFrame;
