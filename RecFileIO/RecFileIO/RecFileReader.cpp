...
void RecFileReader::getAgentOrientationAtTime( unsigned int agentIndex, float time, float &dirx, float &diry, float &dirz )
{
	...
	if ((abs(v1.y) >= 0.00001f) && (abs(v2.y) >= 0.00001f)) {
...
//Adam's Extern C Functions

extern "C"{

//Call the reader's constructor
__declspec(dllexport) RecFileReader* CreateRecReader(char* filename){
    return new RecFileReader(filename);
}

//Call the reader's destructor
__declspec(dllexport) void DisposeRecReader(RecFileReader* reader){
    if(reader != NULL){
        delete reader;
        reader = NULL;
    }
...
//getNumAgents()
__declspec(dllexport) unsigned int CallGetNumAgents(RecFileReader* reader){
    if(reader != NULL){
        return reader->getNumAgents();
    }
    return -1;
}

//getNumObstacles()
__declspec(dllexport) unsigned int CallGetNumObstacles(RecFileReader* reader){
    if(reader != NULL){
        return reader->getNumObstacles();
    }
    return -1;
}
...
__declspec(dllexport) float GetObstacleXMin(RecFileReader* reader, unsigned int index){
    if(reader != NULL){
        return reader->getObstacleBoundsAtFrame(index, 0).xmin;
    }
    return -1;
}

__declspec(dllexport) float GetObstacleXMax(RecFileReader* reader, unsigned int index){
    if(reader != NULL){
        return reader->getObstacleBoundsAtFrame(index, 0).xmax;
    }
    return -1;
}

__declspec(dllexport) float GetObstacleZMin(RecFileReader* reader, unsigned int index){
    if(reader != NULL){
        return reader->getObstacleBoundsAtFrame(index, 0).zmin;
    }
    return -1;
}

__declspec(dllexport) float GetObstacleZMax(RecFileReader* reader, unsigned int index){
    if(reader != NULL){
        return reader->getObstacleBoundsAtFrame(index, 0).zmax;
    }
    return -1;
}
...
__declspec(dllexport) float GetCenterOfMassX(RecFileReader* reader, unsigned int agentIndex, float timeStamp){
    if(reader != NULL){
		return reader->getAgentLocationAtTime(agentIndex, timeStamp).x;
    }
    return -1;
}

__declspec(dllexport) float GetCenterOfMassZ(RecFileReader* reader, unsigned int agentIndex, float timeStamp){
    if(reader != NULL){
		return reader->getAgentLocationAtTime(agentIndex, timeStamp).z;
    }
    return -1;
}

__declspec(dllexport) float GetOrientationX(RecFileReader* reader, unsigned int agentIndex, float timeStamp){
    if(reader != NULL){
		return reader->getAgentOrientationAtTime(agentIndex, timeStamp).x;
    }
    return -1;
}

__declspec(dllexport) float GetOrientationZ(RecFileReader* reader, unsigned int agentIndex, float timeStamp){
    if(reader != NULL){
		return reader->getAgentOrientationAtTime(agentIndex, timeStamp).z;
    }
    return -1;
}

__declspec(dllexport) float GetTargetX(RecFileReader* reader, unsigned int agentIndex, float timeStamp){
    if(reader != NULL){
		return reader->getAgentGoalAtTime(agentIndex, timeStamp).x;
    }
    return -1;
}

__declspec(dllexport) float GetTargetZ(RecFileReader* reader, unsigned int agentIndex, float timeStamp){
    if(reader != NULL){
		return reader->getAgentGoalAtTime(agentIndex, timeStamp).z;
    }
    return -1;
}
}