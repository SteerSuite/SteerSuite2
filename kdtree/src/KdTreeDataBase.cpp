...
void KdTreeDataBase::getItemsInRange(std::set<SpatialDatabaseItemPtr> & neighborList, float xmin, float xmax, float zmin, float zmax, SpatialDatabaseItemPtr exclude)
{
	...
	this->_spatialDatabase->computeAgentNeighbors(agent, 100.0f );
	...
	agent->obstacleNeighbors_.clear();
	this->_spatialDatabase->computeObstacleNeighbors(agent, 10.0f );
...
bool KdTreeDataBase::trace(const Util::Ray & r, float & t, SpatialDatabaseItemPtr &hitObject, SpatialDatabaseItemPtr exclude, bool excludeAgents)
{
	...
	if (!excludeAgents)
	{
		std::set<SteerLib::AgentInterface *> n;
		for (  std::vector<std::pair<float, const SteerLib::AgentInterface *> >::iterator item = agent->agentNeighbors_.begin();
				item != agent->agentNeighbors_.end(); item++)
		{
			n.insert(const_cast<SteerLib::AgentInterface *>((*item).second));
		}
		for (  std::set<SteerLib::AgentInterface *>::iterator item = n.begin();
				item != n.end(); item++)
		{
			// std::cout << "Looking for obstacle colissions" << std::endl;
			// SteerLib::ObstacleInterface * tmp_obs = agent->obstacleNeighbors_.at(o);
			Ray tempRay;
			float temp_t;
			...
			if ( (*item)->intersects(tempRay, temp_t ) && ((*item) != exclude) )
			{ // There is a potential intersection,
				if ( (temp_t < current_closest_dist) )
	...
	std::set<SteerLib::ObstacleInterface *> n2;
	for (  std::vector<std::pair<float, const SteerLib::ObstacleInterface *> >::iterator item = agent->obstacleNeighbors_.begin();
			item != agent->obstacleNeighbors_.end(); item++)
	{
		n2.insert(const_cast<SteerLib::ObstacleInterface *>((*item).second));
	}
	for (  std::set<SteerLib::ObstacleInterface *>::iterator item = n2.begin();
			item != n2.end(); item++)
	{
		// std::cout << "Looking for obstacle colissions" << std::endl;
		// SteerLib::ObstacleInterface * tmp_obs = agent->obstacleNeighbors_.at(o);
		...
		if ( (*item)->intersects(tempRay, temp_t ) && ((*item) != exclude) )
		{ // There is a potential intersection,
			if ( temp_t < r.maxt && (temp_t < current_closest_dist))
			{ // check if intersection is before ray time limit
				current_closest_dist = temp_t;
				hitObject = (*item);
				found_hit=true;
